...
function convertToRomanNumerals(num) {
  ...
  if (num <= 19) return `X${numbers[num % 10]}`;
  if (num === 20) return `XX`;
  if (num <= 29) return `XX${numbers[num % 10]}`;
  if (num === 30) return 'XXX';
  if (num <= 39) return `XXX${numbers[num % 10]}`;
  return false;
}
...
function isPalindrome(str) {
  ...
  const middleSecondHalf = str.length % 2 === 0 ? middle : middle + 1;

  for (let i = 0; i < middle; i += 1) {
    firstHalf += str[i];
  }
  for (let i = str.length - 1; i >= middleSecondHalf; i -= 1) {
    secondHalf += str[i];
  }
...
function getSpiralMatrix(size) {
  ...
  let [startCol, endCol] = [0, matrix.length - 1];
...
function sortByAsc(arr) {
  function partition(array, firstIndex, lastIndex) {
    ...
    for (let i = firstIndex; i < lastIndex; i += 1) {
      if (arrayLink[i] < pivot) {
        [arrayLink[i], arrayLink[pivotIndex]] = [
...
function shuffleChar(str, iterations) {
  ...
  for (let index = 0; index < iterations; index += 1) {
    ...
    for (let i = 0; i < strModify.length; i += 1) {
      if (i % 2 === 0) {
        res1 += strModify[i];
      } else {
        res2 += strModify[i];
...
function getNearestBigger(number) {
  ...
  for (let i = numArr.length - 1; i > index; i -= 1) {
  ...
  let j = 0;
  while (subArr[j] <= numArr[index]) {
    j += 1;
  }

  [numArr[index], subArr[j]] = [subArr[j], numArr[index]];
  numArr.splice(index + 1);

  return Number([...numArr, ...subArr].join(''));
}
